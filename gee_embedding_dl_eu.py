...
CHUNKS = {
    # "bands_01_22": ALL_BANDS[0:22],
    "bands_23_44": ALL_BANDS[22:44],
    "bands_45_63": ALL_BANDS[44:63]
}
...
# ========== Grid Creation and Export ==========
def create_and_export_grid():
    logging.info("Creating EPSG:3035 grid over valid wetland extent...")
    wetland = ee.Image("projects/ee-gmkovacs/assets/ext_wetland_2018_v2021_nw").lt(22).selfMask()
    reducer = ee.Reducer.countEvery()
    valid_extent = wetland.reduceToVectors(
        ...
        maxPixels=1e13,
        bestEffort=True
    ).geometry().dissolve(100).buffer(-5000)

    crs = "EPSG:3035"
    grid = valid_extent.coveringGrid(crs, GRID_SIZE).map(lambda feat: feat.buffer(distance=-1))
    asset_id = f"{ASSET_FOLDER}europe_wetland_grid_{GRID_SIZE}m_epsg3035"

    try:
        ee.data.getAsset(asset_id)
        logging.info(f"Grid asset already exists: {asset_id}")
    except ee.EEException:
        logging.info(f"Exporting grid to asset: {asset_id}")
        task = ee.batch.Export.table.toAsset(
            collection=grid,
            description="europe_grid_3035",
            assetId=asset_id
        )
        task.start()
        while task.active():
            logging.info("Waiting for export task to finish...")
            time.sleep(10)
...
def check_and_download(params):
    ...
    if os.path.exists(outp):
        try:
            with rasterio.open(outp) as dst:
                # Check if it has the expected number of bands
                if dst.count == len(band_list) + 1:
                    return None  # Valid file exists, skip download
                else:
                    logging.warning(f"[{i}] File exists but has incorrect band count: {dst.count}")
        except Exception as e:
            logging.warning(f"[{i}] File exists but failed to open: {e}")
        # If file is invalid, proceed to re-download
...
# ========== Main ==========
def main():
    create_and_export_grid()
    logging.info(f"Loading pre-exported grid from: {GRID_ASSET}")
    ...
    for chunk_name, band_list in CHUNKS.items():
        ...
        # Prepare (i, feature, band_list, chunk_name)
        all_params = [
        ...
        logging.info(f"Chunk {chunk_name}: {succeeded}/{size} tiles downloaded successfully.")
